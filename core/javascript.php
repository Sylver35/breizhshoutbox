...
class javascript
{
	...
	private function auth_to_javascript($data)
	{
		...
		$settings_auth = [
			...
			'isUser'			=> $this->work->return_js_bool((bool) $data['is_user']),
			'isFounder'			=> $this->work->return_js_bool((bool) $data['founder']),
			'isGuest'			=> $this->work->return_js_bool((bool) $data['guest']),
			'isRobot'			=> $this->work->return_js_bool((bool) $data['is_bot']),
			'isPriv'			=> $this->work->return_js_bool((bool) $data['private']),
			'isPopup'			=> $this->work->return_js_bool((bool) $data['popup']),
			'rulesOk'			=> $this->work->return_js_bool((bool) $rules),
			'rulesOpen'			=> $this->work->return_js_bool((bool) $rules_open),
			'isMobile'			=> $this->work->return_js_bool((bool) $data['is_mobile']),
			'topBar'			=> $this->work->return_js_bool((bool) $data['shout_bar_option' . $data['sort_p']]),
			'toBottom'			=> $this->work->return_js_bool((bool) $data['shout_defil' . $data['sort_p']]),
			...
			'buttonIp'			=> $this->work->return_js_bool((bool) $this->config['shout_see_button_ip']),
			'buttonCite'		=> $this->work->return_js_bool((bool) $this->config['shout_see_cite']),
			'endClassBg'		=> $this->work->return_js_bool((bool) $this->config['shout_button_background' . $data['sort_p']]),
			'purgeOn'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('a_shout' . $data['sort_perm'])),
			'onlineOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_gets('u_viewprofile', 'a_user', 'a_useradd', 'a_userdel')),
			'postOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_post')),
			'limitPost'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_limit_post')),
			'smiliesOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_smilies')),
			'imageOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_image')),
			'colorOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_color')),
			'bbcodeOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_bbcode')),
			'charsOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_chars')),
			'popupOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_popup')),
			'formatOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_bbcode_change') && $data['is_user']),
			'privOk'			=> $this->work->return_js_bool((bool) $this->auth->acl_get('u_shout_priv') && $data['is_user']),
	...
	private function settings_to_javascript($data)
	{
		$list_string = [
			...
			'popupUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_popup')),
			'configUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_configshout')) . '?user_id=' . $data['user_id'],
			'checkUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'check'])),
			'viewUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'view'])),
			'postUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'post'])),
			'smilUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'smilies'])),
			'smilPopUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'smilies_popup'])),
			'onlineUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'online'])),
			'soundUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'action_sound'])),
			'rulesUrl'			=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'rules'])),
			'postingUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'posting'])),
			'questionUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'question'])),
			'creatorUrl'		=> ($data['creator']) ? $this->work->remove_sid((string) $this->helper->route('sylver35_smilecreator_controller')) : '',
		];
		if ($data['is_user'])
		{
			$list_string = array_merge($list_string, [
				'privUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_private')),
				'purgeUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'purge'])),
				'purgeBotUrl'	=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'purge_robot'])),
				'actUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'action_user'])),
				'actPostUrl'	=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'action_post'])),
				'actDelUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'action_del'])),
				'actDelToUrl'	=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'action_del_to'])),
				'actRemoveUrl'	=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'action_remove'])),
				'citeUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'cite'])),
				'ubbcodeUrl'	=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'user_bbcode'])),
				'persoUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'charge_bbcode'])),
				'deleteUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'delete'])),
				'editUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'edit'])),
				'dateUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'date_format'])),
				'authUrl'		=> $this->work->remove_sid((string) $this->helper->route('sylver35_breizhshoutbox_ajax', ['mode' => 'auth'])),
			]);
		}
